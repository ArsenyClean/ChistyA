...
public class UserStore {
        ...
        try {
            ...
            if (connection != null) {
                connection.createStatement().execute("CREATE TABLE IF NOT EXISTS users " +
                        "(id SERIAL PRIMARY KEY, name varchar(30), login varchar(30), email varchar(30)," +
                        " password varchar(30), role varchar(30)," +
    ...
    public User findUserLogPassw(String login, String password) {
        User userFinal = null;
    ...
    public User getById(Integer id) {
        ...
        try {
            ...
            ResultSet rs;
            pr = connection.prepareStatement("SELECT id, name, login, email, password, role, date_creation from users where id=(?);");
            pr.setInt(1, id);
            rs = pr.executeQuery();
            connection.commit();
            if(rs.isBeforeFirst()) {
    ...
    public void add(User user) {
        try {
            PreparedStatement pr;
            pr = connection.prepareStatement("insert into users(name, login, email, password, role, date_creation) values (?,?,?,?,?,?) RETURNING id;");
            pr.setString(1, user.getName());
            pr.setString(2, user.getLogin());
            pr.setString(3, user.getEmail());
            pr.setString(4, user.getPassword());
            pr.setString(5, user.getRole());
            pr.setString(6, user.getCreateDate());
    ...
    public boolean update(Integer id, String name, String login, String email, String password, String role) {
        boolean result = false;
        try {
            PreparedStatement pr;
            pr = connection.prepareStatement("UPDATE users set name=(?), login=?, email=(?), password=?, role=?, where id=? returning id;");
            pr.setString(1, name);
